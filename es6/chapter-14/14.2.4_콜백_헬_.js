...
// 세 가지 파일의 콘텐츠를 읽고, 60초가 지난 다음, 이들을 결합해 네 번째 파일에 기록하기
const fs = require('fs');
...
// 이런 코드를 [콜백 헬]이라 부릅니다.
// 중괄호로 둘러싸여 끝없이 중첩된 삼각형의 코드 블록들은 마치 버뮤다의 삼각지대처럼 보일 지경입니다.
// 더 골치 아픈 문제는 에러 처리입니다.
// 이 에제에서는 에러를 기록하기만 했지만, 예외를 일으키려 했다면 더더욱 골치가 아팠을 겁니다.
const fs = require('fs');
...
// 동작하지 않는다는 것만 빼면 말입니다. 직접 실행해 보십시오.
// 예외 처리가 의도대로 동작하지 않는 이유는 try...catch 블록은 [같은 함수] 안에서만 동작하기 때문입니다.
// try...catch 블록은 readSketchyFile 함수 안에 있지만, 정작 예외는 fs.readFile이 콜백으로 호출하는 
// 익명 함수 안에서 일어났습니다.

// 또한 콜백이 우연히 두 번 호출되거나, 아예 호출되지 않는 경우를 방지하는 안전장치도 없습니다.
// 콜백이 정확히 한 번만 호출될 것을 가정하고 코드를 작성한다면, 애석하지만 자바스크립트는 그런 걸 보장하지 않습니다.
